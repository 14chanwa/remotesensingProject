...
\begin{figure}[ht]
  \centering
  \includegraphics[height=9.5cm]{drawings/rslf__fine__to__coarse_8hpp__incl.pdf}
  \caption{Header inclusion dependancy tree}
...
int main() {
    ...
    // The last flag should be set according to the data type: float is UNCHANGED
    ...
    float d_max = 4.0;
    int dim_d = 120;
    // Use FineToCoarse_1ch for monochannel, FineToCoarse_3ch for trichannel images 
...
\end{itemize}
These are template classes which internal structures depend on the number of channels of the image. Aliases are defined such as \verb#FineToCoarse_1ch#, \verb#Depth2DComputer_3ch#... are defined for convenience. The algorithm only supports monochannel and trichannel images.
